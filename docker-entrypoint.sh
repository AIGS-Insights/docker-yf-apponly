...
mkdir -p "${INSTALL_PATH}" && chmod a+w "${INSTALL_PATH}"
...
if [ -f "$COMPLETION_FILE" ]; then
    ...
    if [ -f "/tmp/custom.properties" ]; then
        ...
        && echo "option.db.dbtype=$JDBC_CONN_TYPE" >> /tmp/custom.properties \
        ...
        if [ ! -z "${JDBC_CONN_HOST}" ]; then
            echo "option.db.hostname=$JDBC_CONN_HOST" >> /tmp/custom.properties
        fi
        if [ ! -z "${JDBC_CONN_PORT}" ]; then
            echo "option.db.port=$JDBC_CONN_PORT" >> /tmp/custom.properties
        fi
        if [ ! -z "${JDBC_CONN_DBNAME}" ]; then
